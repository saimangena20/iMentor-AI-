...
async function selectLLM(query, context) {
  ...
  // If the user prefers Ollama, check if it's actually available.
  // If not, temporarily switch to Gemini for this request.
  if (preferredProvider === 'ollama') {
    const isOllamaUp = await checkOllamaHealth(user?.ollamaUrl || process.env.OLLAMA_API_BASE_URL);
    if (!isOllamaUp) {
      console.warn('[LLMRouter] User preferred Ollama, but it is unreachable. Temporarily falling back to Gemini for this request.');
      preferredProvider = 'gemini'; // Temporary override
    }
  }

  console.log(`[LLMRouter] Selecting LLM for query. Effective provider: ${preferredProvider}`);
  ...
  console.error(`[LLMRouter] CRITICAL: No models found for preferred provider '${preferredProvider}' AND no system default! Using hardcoded fallback for provider.`);

  // If we have to fall back, at least respect the user's chosen provider.
  // Use the environment variable for the default Ollama model if the provider is Ollama.
  const fallbackModelId = preferredProvider === 'ollama'
    ? (process.env.OLLAMA_DEFAULT_MODEL || 'qwen2.5:14b-instruct')
    : 'gemini-1.5-flash';

  return {
    chosenModel: { modelId: fallbackModelId, provider: preferredProvider },
    logic: 'absolute_hardcoded_fallback_per_provider'
  };
}
