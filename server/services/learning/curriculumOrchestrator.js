...
async function generateModulesForGoal(goal, user, context) {
    ...
    let responseText;
    try {
        responseText = await llmService.generateContentWithHistory([], prompt, null, llmOptions);
    } catch (error) {
        console.error(`[CurriculumOrchestrator] LLM service error, using fallback modules:`, error.message);
        // Fallback modules when LLM fails
        responseText = JSON.stringify({
            modules: [
                { title: "Foundation & Core Concepts", objective: "Understand the fundamental principles and terminology", activity: { type: "direct_answer", suggestedPrompt: "Explain the core concepts and fundamentals" } },
                { title: "Practical Implementation", objective: "Apply knowledge through hands-on examples and exercises", activity: { type: "code_executor", suggestedPrompt: "Show me practical examples with code" } },
                { title: "Advanced Topics", objective: "Explore advanced techniques and optimization", activity: { type: "direct_answer", suggestedPrompt: "What are the advanced topics and best practices?" } },
                { title: "Real-World Applications", objective: "Learn how this is used in production environments", activity: { type: "web_search", suggestedPrompt: "Find real-world examples and case studies" } }
            ]
        });
    }
...
async function createLearningPath(userId, goal, context = {}) {
    ...
    try {
        ...
        analysisResult = JSON.parse(analysisResponseText.match(/\{[\s\S]*\}/)[0]);
    } catch (error) {
        // Fallback: Assume most goals are specific to allow workflow to continue
        console.log(`[CurriculumOrchestrator] LLM analysis failed, using fallback (assuming goal is specific)`);
        analysisResult = { isSpecific: true };
    }
    ...
    if (analysisResult.isSpecific === false) {
        ...
        try {
            ...
            questionsResult = JSON.parse(questionsResponseText.match(/\{[\s\S]*\}/)[0]);
        } catch (error) {
            // Fallback questionnaire
            questionsResult = {
                questions: [
                    { questionText: "What specific aspect would you like to focus on?", type: "text_input" },
                    { questionText: "What is your current experience level?", type: "multiple_choice", options: ["Beginner", "Intermediate", "Advanced"] }
                ]
            };
        }
