            ...
            <form onSubmit={handleSubmit} className="flex items-end gap-2">
                    ...
                    <AnimatePresence>
                        {isMenuOpen && (
                            ...
                            >
                                ...
                                {/* Mobile-only Utility items moved from main row */}
                                <div className="sm:hidden border-t border-border-light dark:border-border-dark mt-1 pt-1">
                                    {isSpeechSupported && (
                                        <button
                                            onClick={() => { listening ? stopListening() : startListening(); setIsMenuOpen(false); }}
                                            className={`w-full text-left flex items-center gap-2 px-3 py-2 text-sm rounded-md transition-colors ${listening ? 'bg-red-500/10 text-red-500' : 'text-text-light dark:text-text-dark hover:bg-gray-100 dark:hover:bg-gray-700'}`}
                                        >
                                            <Mic size={16} /> {listening ? 'Stop Voice Input' : 'Voice Input'}
                                        </button>
                                    )}
                                    <button
                                        onClick={() => { handleRequestPromptCoaching(); setIsMenuOpen(false); }}
                                        className="w-full text-left flex items-center gap-2 px-3 py-2 text-sm rounded-md text-amber-600 dark:text-amber-400 hover:bg-gray-100 dark:hover:bg-gray-700"
                                    >
                                        <Sparkles size={16} /> Prompt Coach
                                    </button>
                                    <button
                                        onClick={() => { setCriticalThinkingEnabled(!criticalThinkingEnabled); setIsMenuOpen(false); }}
                                        className={`w-full text-left flex items-center gap-2 px-3 py-2 text-sm rounded-md transition-colors ${criticalThinkingEnabled ? 'bg-purple-500/10 text-purple-600' : 'text-text-light dark:text-text-dark hover:bg-gray-100 dark:hover:bg-gray-700'}`}
                                    >
                                        <Brain size={16} /> Critical Thinking ({criticalThinkingEnabled ? 'ON' : 'OFF'})
                                    </button>
                                    <button
                                        onClick={() => { setIsKgRealtimeEnabled(!isKgRealtimeEnabled); setIsMenuOpen(false); toast.success(`Live Concept Map is now ${!isKgRealtimeEnabled ? 'ON' : 'OFF'}.`); }}
                                        className={`w-full text-left flex items-center gap-2 px-3 py-2 text-sm rounded-md transition-colors ${isKgRealtimeEnabled ? 'bg-teal-500/10 text-teal-600' : 'text-text-light dark:text-text-dark hover:bg-gray-100 dark:hover:bg-gray-700'}`}
                                    >
                                        <Share2 size={16} /> Live Concept Map ({isKgRealtimeEnabled ? 'ON' : 'OFF'})
                                    </button>
                                </div>
                            </motion.div>
                        )}
                ...
                {isSpeechSupported && (
                    <IconButton
                        ...
                        size="md"
                        className={`p-2 hidden sm:flex ${listening ? 'text-red-500 animate-pulse' : 'text-text-muted-light dark:text-text-muted-dark hover:text-primary'}`}
                        disabled={isLoading}
                    />
                )}
                ...
                <IconButton
                    ...
                    variant="ghost"
                    size="md"
                    className="p-2 hidden sm:flex text-amber-500 hover:text-amber-600 dark:text-amber-400 dark:hover:text-amber-300"
                ...
                <IconButton
                    ...
                    variant="ghost"
                    size="md"
                    className={`p-2 hidden sm:flex ${isKgRealtimeEnabled ? 'text-teal-500' : 'text-text-muted-light dark:text-text-muted-dark hover:text-primary'}`}
                    disabled={isLoading}
                />
                {/* --- END FIX --- */}
